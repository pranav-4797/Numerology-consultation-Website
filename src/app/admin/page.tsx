...
export default function AdminDashboard() {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 lg:p-8 overflow-visible">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {visibility && sectionLabels.map(({ key, label, icon }) => {
            ...
            return (
              <div
                key={key}
                className={`flex items-center justify-between p-3 sm:p-4 rounded-xl border transition-all duration-300 min-w-0 ${
                  isEnabled
              ...
              >
                    ...
                    <p className="text-[11px] text-text/40">
                      {isEnabled ? (
                        <span className="flex items-center gap-1 text-green-600">
                          <Eye className="w-3 h-3 flex-shrink-0" /> Visible to users
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 text-red-500">
                          <EyeOff className="w-3 h-3 flex-shrink-0" /> Hidden from users
                        </span>
                      )}
                    </p>
                  </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`relative w-12 h-7 rounded-full transition-colors duration-300 flex-shrink-0 ml-2 ${
